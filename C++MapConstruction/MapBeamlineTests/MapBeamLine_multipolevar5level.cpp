	...
	if (nbthreads >= 16) {
		...
		for (int i = 0; i < 8; i ++)
		 ...
		 #pragma omp parallel for shared(Res) schedule(static) num_threads(8)
                for (int i = 0; i < nbthreads; i ++) {
                        int index =  omp_get_thread_num();
			Res1[index] = Res[i].parallel_composition( Res1[index]);
		...
		for (int i = 0; i < 4; i ++) {
			int index =  omp_get_thread_num();
			Res2[index] = Res1[i] .parallel_composition(Res2[index]);
		...
		#pragma omp parallel for shared(Res) schedule(static) num_threads(2)
                for (int i = 0; i < 4; i ++) {
                        int index =  omp_get_thread_num();
                        Res3[index] = Res2[i] .parallel_composition(Res3[index]);
		...
		R = Res3[1].parallel_composition(Res3[0]);
		...
		for (int i = 1; i < nbthreads; i ++) {
			  R = Res[i].parallel_composition(R);
		}
	}
	double end = omp_get_wtime();
	cout << 1000 *(end - start) << endl;
