...
MapBeamLine::MapBeamLine(Twiss t, int order, int nbthreads, int fmultipole, bool strpl) {
	...
	if (strpl) {
		...
		for (int i = 0; i < size; i ++) {
			...
			if (t.elems[i]["KEYWORD"].compare(sextupole) == 0 ||
				t.elems[i]["KEYWORD"].compare(octupole) == 0 )
                                Res[index] = mp[i].parallel_composition(Res[index]);
	...
	if (nbthreads >= 8) {
		Polmap<double>* Res2 = new Polmap<double>[2];
		Res2[0] = R;
		Res2[1] = R; 
		...
		for (int i = 0; i < nbthreads; i ++) {
		//cout << (i - 1) << endl; 
		//for (unordered_map<string, Polynom<double>>:: iterator it = Res[i - 1].pols.begin(); it != Res[i - 1].pols.end(); it ++)
                //	cout << it->first << " " <<  (it->second).terms.size() << endl;

		int index =  omp_get_thread_num();
		Res2[index] = Res[i].parallel_composition(Res2[index]);
		...
		R = Res2[1].parallel_composition(Res2[0]);
